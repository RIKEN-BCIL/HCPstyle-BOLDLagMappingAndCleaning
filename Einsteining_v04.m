function Einsteining_v04( Sdir, TR, Nvols, MaxLag, MinR, Fixed, Sm, onlyLag)
...
%
% Example usage:
% Einsteining_v03_REST( Sdir, '0.80', '375', '9', '0.3', '0', '8', '0')
...
disp( [ 'drLag4Drev6_nooverwrite( ''cat' num2str( length( Runs)) ''',' num2str( TR) ',''' out ''',' num2str( MaxLag) ',' num2str( MinR) ',' num2str( Fixed) ',' num2str( Sm) ')'])

% Lag = drLag4Drev5_hcp( [ 'cat' num2str( length( Runs)) ], num2str( TR), out, num2str( MaxLag), num2str( MinR), num2str( Fixed), num2str( Sm));
Lag = drLag4Drev6_nooverwrite( [ 'cat' num2str( length( Runs)) ], num2str( TR), out, num2str( MaxLag), num2str( MinR), '1', num2str( Sm));
...
S = system( [ 'niimath All.nii -add SumMean ' name '.nii']);
...
%		W = W( round( N/3):round( N/3)+N-1);		
		...
		dlmwrite( 'Reg.txt', Reg, 'delimiter','\t', 'precision','%.4f')
