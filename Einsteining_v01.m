function Einsteining_v01( Sdir, TR, Nvols, MaxLag, MinR, Fixed, Sm, onlyLag)
...
Lag = drLag4Drev4_hcp( [ 'cat' num2str( length( Runs)) ], num2str( TR), out, num2str( MaxLag), num2str( MinR), num2str( Fixed), num2str( Sm));
...
for r=1:size( Runs, 1)
	...
	%	drDeperf_hcp_seed_fixit( Deps, Runs{ r}, Lag, TR, r, length( Runs))
		drDeperf_hcp_seed( Runs{ r}, Lag, TR, r, length( Runs))
	end
