function dir = drLag4Drev7_longTR( name, TR, vols, PosiMax, THR, FIXED, Smooth, range)
...
%	PosiMax: Determines tracking range in SECOND -PosiMax - +PosiMax 
...
%   Smooth: intensity of smoothing in mm FWHM. Eight mm would be safe with short acquisitions <20 min.
...
MaxLag = PosiMax*2;		% lag range in sec
ULfreq = 1/MaxLag*.9; % <= upper limit of frequency; fixed value such as 0.1Hz would do

reso = 1; % Lag resolution: smaller the value, the more memory load
Nseeds = PosiMax*2/reso + 1;% number of regions/seed timeseries; fixed value such as 0.1Hz would do

limit = ceil( PosiMax);
Cmap = jet( Nseeds);

if FIXED==0
	dir = [ pwd '/Lag_rec_' num2str( MaxLag) 'sec_thr' num2str( round( 10*THR)) '_sm' num2str( round( Sm)) '_' name];
else
	dir = [ pwd '/Lag_fix_' num2str( MaxLag) 'sec_thr' num2str( round( 10*THR)) '_sm' num2str( round( Sm)) '_' name];
end

if exist( [ dir '/MaxR.nii'], 'file')
	disp( 'Overwriting LagMap...')
...
if ~exist( [ 'sm' Smooth '_' num2str( MaxLag) 'sec.nii'], 'file')
	disp('Preparing the data...') %-------------------------------------------

	S = [ fslmath ' ' vols ' -Tstd SD.nii']; if system( S), error, end
	S = [ fslmath ' ' vols ' -Tmean Tmean']; if system( S), error, end
	S = system( [ fslmath ' Tmean -thrp 15 Mask']); % Mask 

	Vm = spm_vol( 'Mask.nii');
	M = spm_read_vols( Vm);
	M( M==0) = NaN;
	M( ~isnan( M)) = 1;
	Vm.fname = 'nanmask.nii';
	spm_write_vol( Vm, M);
	...
	Mean = mean( S, 4);
	S = 100*S./repmat( Mean.*M, [1 1 1 size( S, 4)]);
	...
	if Sm>0
		...
		drSmooth4D( temp, Sm);
		disp('Filtering...')
		S = system( [ fslmath ' ' WD '/sm' Smooth '_nanvols.nii -nan -bptf ' ...
		num2str( 1/( 0.008*2.35*TR)) ' ' num2str( 1/( ULfreq*2.35*TR)) ' sm' Smooth '_' num2str( MaxLag) 'sec.nii']);
		if S, error, end
	else
		disp('Filtering...')
		S = system( [ fslmath ' ' WD '/nanvols.nii -nan -bptf ' ...
		num2str( 1/( 0.008*2.35*TR)) ' ' num2str( 1/( ULfreq*2.35*TR)) ' sm' Smooth '_' num2str( MaxLag) 'sec.nii']);
		if S, error, end
	end
end
...
V = spm_vol( [ 'sm' Smooth '_' num2str( MaxLag) 'sec.nii']);
...
for p=1:limit
	...
	Lag( I==2) = -p;
	Seeds = [ Seeds SeedD(:) ];
	fprintf( 1,'\b\b\b\b\b\b\b\b\b%0+5.1f sec', -p*reso)
...
function out = drMatCell( in)
if iscell( in), out = in; return, end
out = [];
for p=1:size( in,1)
	out = [ out; { deblank( in( p,:))}];
end
return
...
if ~ischar(RPATH) error( RPATH); end; 
...
function out = drErode1( in, MaxLag)

if nargin==0
	in = spm_select;
end
%[ outD, N, E] = fileparts( in);
if size( in,1)>1
	for p=1:size( in,1)
		drErode1( deblank( in( p,:)), MaxLag)
	end
else
