function Einsteining_v06_monkey( Sdir, TR, Nvols, MaxLag, MinR, Fixed, Sm, onlyLag)
...
disp( [ 'drLag4Drev7_saru_noredo( ''cat' num2str( length( Runs)) ''',' num2str( TR) ',''' out ''',' num2str( MaxLag) ',' num2str( MinR) ',' num2str( Fixed) ',' num2str( Sm) ')'])

% recursive lag tracking: Lag = drLag4Drev7_monkey( [ 'cat' num2str( length( Runs)) ], num2str( TR), out, num2str( MaxLag), num2str( MinR), '0', num2str( Sm));
Lag = drLag4Drev7_monkey( [ 'cat' num2str( length( Runs)) ], num2str( TR), out, num2str( MaxLag), num2str( MinR), '1', num2str( Sm));
...
% Deperfusioning needs a further optimization for each dataset. Email me if you are interested!
