...
%	vols: File name of single 4D BOLD data to process.
...
%	save S.mat S 
	...
	if Sm>0
		disp('Smoothing...')
		temp = drDir( [ WD '/nanvols.nii']);
		drSmooth4D( temp, Sm);
		disp('Filtering...')
		S = system( [ fslmath ' ' WD '/sm' Smooth '_nanvols.nii -nan -bptf ' ...
		num2str( 1/( 0.008*2.35*TR)) ' ' num2str( 1/( ULfreq*2.35*TR)) ' sm' Smooth '_' num2str( MaxLag) 'TR.nii']);
		if S, error, end
	else
		disp('Filtering...')
...
V = spm_vol( [ 'sm' Smooth '_' num2str( MaxLag) 'TR.nii']);
