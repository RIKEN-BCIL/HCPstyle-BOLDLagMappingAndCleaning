function Einsteining_v06_hireso_monkey( Sdir, TR, Nvols, MaxLag, MinR, Fixed, Sm, onlyLag)
...
%
% Example usage:
% Einsteining_v05( Sdir, '0.72', '1200', '9', '0.3', '0', '8', '0')
...
a = which( 'cfg_util');
if isempty( a)
	addpath( [ fileparts( which( 'spm')) '/matlabbatch/'])
end
...
%try
	...
	Runs = [];
	for p=1:size( in, 1)
		if strfind( in( p,:), 'BOLD_')*~length( strfind( in( p,:), 'CONCAT'))*~length( strfind( in( p,:), '@'))
			...
			if isempty( Vol)
				...
				if isempty( Vol)
					...
					if system( [ 'fslchfiletype NIFTI_GZ ' Vol]), error, end
					Vol = spm_select( 'FPList', deblank( in( p,:)), [ '^' Rname '.nii.gz$']);
					Runs = [ Runs; { Vol}];
	...
	if isempty( Runs), save all.mat, error( 'no data'), end
...
%	!sudo mkdir Lag_concat
	mkdir Lag_concat
%	!sudo chmod 777 -R Lag_concat
end
cd Lag_concat

save Runs.mat Runs Ref

if ~exist( [ 'hresREST' num2str( length( Runs)) 'run.nii.gz'], 'file')
	zRuns = Runs;
...
%		if size( zfile,1) == 0
			...
			disp( ['Motion scrubbing Run #' num2str( p)])
			%			if system( [ fslmath ' ' Runs{ p} ' -subsamp2offc z' fname( 1:end-7)]), error, end

			if fname( end)=='z'
				rnd = num2str( round( rand( 1)*1000000000));
				if system( [ 'gunzip -c ' Runs{ p} ' > /tmp/' rnd '.nii']), error, end
				Y = spm_read_vols( spm_vol( [ '/tmp/' rnd '.nii']));
			else
				Y = spm_read_vols( spm_vol( fname));
			end
			Siz = size( Y);
		...
		%	writematrix( out, ['regout' num2str( p) '.txt'],'Delimiter','tab')
			...
			setenv( 'FSLOUTPUTTYPE', 'NIFTI_GZ')
			S = ['fsl_regfilt -i ' Runs{ p} ' -d regout' num2str( p) '.txt -f "' RegN '" -v -o mreg_H' fname( 1:end-7) ];
			if system( S), error, end
			zfile = spm_select( 'FPList', pwd, [ '^mreg_H' fname( 1:end-7) '.*']);
%		end
		zRuns{ p} = zfile( end,:);
	end
	
	out = drMerge4D( [ 'hresREST' num2str( length( Runs)) 'run'], TR, zRuns, fslmath)
else
	out = spm_select( 'FPList', pwd, ['^hresREST' num2str( length( Runs)) '.*.nii.gz']);
end

!rm zBOLD*.nii

disp( [ 'drLag4Drev7_monkey( ''hres_cat' num2str( length( Runs)) ''',' num2str( TR) ',''' out ''',' num2str( MaxLag) ',' num2str( MinR) ',' num2str( Fixed) ',' num2str( Sm) ')'])

Lag = drLag4Drev7_monkey( [ 'hres_cat' num2str( length( Runs)) ], num2str( TR), out, num2str( MaxLag), num2str( MinR), '1', num2str( Sm));
...
rLag = drReslice( Ref, [ Lag '/LagMap.nii'], 2);
movefile( rLag, 'srLagMap.nii')
...
for r=1:size( Runs, 1)
	...
	disp( [ 'drDeperf_hcp_seed( ''' Runs{ r} ', ' Lag ', ' num2str( TR) ', ' num2str( r) ', ' num2str( length( Runs)) ', fslmath, ' num2str( reso)])
	save All.mat
	drDeperf_hcp_seed( Runs{ r}, Lag, TR, r, length( Runs), fslmath, reso)
end
...
function drDeperf_hcp_seed( orig_vols, Lag, TR, section, Nruns, fslmath, reso) 
...
%	
%	reso: Lag resolution in second (set to .5 by default)
%
...
load Seeds.mat
MaxLag =  ( size( Seeds, 2)-1)/2; % in reso

Nvols = length( Seeds)/Nruns
if Nvols ~= dr4Dlen( orig_vols)
	disp( [ Nvols dr4Dlen( orig_vols)])
	error
end
...
S = system( [ fslmath ' ' orig_vols ' -Tmean out.nii.gz' ]);
...
%		W = W( round( N/3):round( N/3)+N-1);
		...
		S = system([ 'fsl_regfilt --in=data_lowcut '...
				' --out=rfmasked' num2str( p) ' --design=Reg.txt -f "1"' ]);
		if S, save all.mat, error, end
		S = system( [ fslmath ' rfmasked' num2str( p) ' -mas Mask rfmasked' num2str( p) ]);
		if S, save all.mat, error, end
...
for p =  1: size( Seeds, 2)
	if exist( ['rfmasked' num2str( p) '.nii.gz'], 'file')
		S = system( [ fslmath ' out -add rfmasked' num2str( p) '.nii.gz out.nii.gz']); if S, error, end
	end
end
...
if system( [ fslmath ' out.nii.gz ' PWD '/' newvols( 1:end-4) '_dep.nii.gz' ]), error, end
...
if size( source, 1)>1
	for p=1:size( source, 1)
		drReslice( ref, deblank( source( p,:)), INT)
	end
	return
end
