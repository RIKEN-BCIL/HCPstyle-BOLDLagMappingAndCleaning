function Einsteining_v06( Sdir, TR, Nvols, MaxLag, MinR, Fixed, Sm, onlyLag)
...
% output directories will be BOLD_REST1_AP_dep, BOLD_REST1_PA_dep,
% BOLD_REST2_AP_dep, BOLD_REST2_PA_dep
%
% Example usage:
% Einsteining_v06( Sdir, '0.72', '1200', '9', '0.3', '0', '8', '0')
%
...
%try
	...
	for p=1:size( in, 1)
		if in( p).isdir
		% Modify the keywords _REST1/_REST2 to specify the source fMRI
		% data. "dep" is avoided to ignore already "deperfusioned" data.
		
		if ( length( strfind( in( p).name, '_REST1'))+length( strfind( in( p).name, '_REST2')))*~length( strfind( in( p).name, 'dep'))*~length( strfind( in( p).name, '@'))
			Rname =in( p).name;
			Vol = drDir( [ Rname '/' Rname '.nii.gz']);
			if isempty( Vol)
				disp( [ 'No data in' Rname])
				return

				% If your data needs to be compressed
				Vol = drDir( [ Rname '/' Rname '.nii']);
				if isempty( Vol)
					error
				else
