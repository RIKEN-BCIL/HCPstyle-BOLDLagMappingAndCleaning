function drDeperf_longTR( orig_vols, Lag, TR, section, Nruns) 
...
global Fdir
Fdir = '/usr/local/fsl-6.0.5.1/bin/'
...
orig_vols = absolute_path( orig_vols);
...
if Nvols/TR ~= dr4Dlen( orig_vols), error, end
...
%Seeds = resample( Seeds, double( round( reso*100)), double( round( TR*100)));
Motodata = resample( Motodata, double( round( reso*100)), double( round( TR*100)));
...
disp('Filtering...')
S = system( [ 'fslmaths ' orig_vols ' -bptf ' ...
...
S = system( [ 'fslmaths ' orig_vols ' -Tmean out.nii.gz' ]);
...
for p = 1:Nseed
	...
	if ~isempty( find( Mask(:)))
		...
		if system( [ 'fslmaths MaskDeperf -mas out Mask']), error, end
		...
		if S, save all.mat, error, end
		S = system( [ 'fslmaths rfmasked' num2str( p) ' -mas Mask rfmasked' num2str( p) ]);
		if S, save all.mat, error, end
...
for p =  1: size( Seeds, 2)
	if exist( ['rfmasked' num2str( p) '.nii.gz'], 'file')
		S = system( [ 'fslmaths out -add rfmasked' num2str( p) '.nii.gz out.nii.gz']); if S, error, end
	end
...
if system( [ 'mv out.nii.gz ' PWD '/dep_' newvols ext '.gz']), error, end
cd( PWD)
...
if S, error, end
out = str2double( out);
if isnan( out), save all.mat, error, end
