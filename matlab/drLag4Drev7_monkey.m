function dir = drLag4Drev7_saru( name, TR, vols, PosiMax, THR, FIXED, Smooth, range)
%
...
MaxLag = PosiMax*2;		% lag range in second
ULfreq = 1/MaxLag/TR*.9; % <= upper limit of frequency; fixed value such as 0.1Hz would do

Nseeds = PosiMax/reso + 1;% number of regions/seed timeseries

limit = ceil( PosiMax/reso);
Cmap = jet( Nseeds);

if FIXED==0
	dir = [ pwd '/Lag_rec_' num2str( PosiMax) 'sec_thr' num2str( round( 10*THR)) '_sm' num2str( round( Sm)) '_' name];
else
	dir = [ pwd '/Lag_fix_' num2str( PosiMax) 'sec_thr' num2str( round( 10*THR)) '_sm' num2str( round( Sm)) '_' name];
end
...
data = spm_select( 'List', pwd, [ '.*_sm' Smooth '_' num2str( MaxLag) 'sec.nii']);

%if ~exist( [ name '_sm' Smooth '_' num2str( MaxLag) 'sec.nii'], 'file')
if isempty( data)
	disp('Preparing the data...') %-------------------------------------------

	S = [ fslmath ' ' vols ' -Tstd SD.nii'];
	if system( S), system( [ 'fslmaths ' vols ' -Tstd SD.nii']), end
	S = [ fslmath ' ' vols ' -Tmean Tmean']; if system( S), error, end
	if system( S), system( [ 'fslmaths ' vols ' -Tmean Tmean.nii']), end
...
%	save S.mat S 
	...
	if Sm>0
		...
		num2str( 1/( 0.008*2.35*TR)) ' ' num2str( 1/( ULfreq*2.35*TR)) ' ' name '_sm' Smooth '_' num2str( MaxLag) 'sec.nii']);
		if S, error, end
	else
		disp('Filtering...')
...
%V = spm_vol( [ name '_sm' Smooth '_' num2str( MaxLag) 'sec.nii']);
...
% sY = Y*1000./repmat( std( Y, [], 1), size( sY, 1),1); 
Seed = nanmean( Y,2);
save RawSeed.mat Seed

Ysd = nanstd( Y,[],1);
Ysd( Ysd==0) = NaN;
Y = 10000*Y./repmat( Ysd, [ size( Y,1) 1]);
Seed = nanmean( Y,2);
...
%figure( 99)
%imagesc( [ Lag1( :,:,26) Lag1( :,:,23); Lag1( :,:,20) Lag1( :,:,17)])
%colormap jet
%drawnow
