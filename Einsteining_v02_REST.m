...
%try
	...
	Runs = [];
	for p=1:size( in, 1)
		if strfind( in( p,:), 'REST')*~length( strfind( in( p,:), 'dep_'))
...
%			end
	...
	out = drMerge4D_LagmapGZ_fsl5( [ 'REST' num2str( length( Runs)) 'run'], TR, zRuns)
else
	out = spm_select( 'FPList', pwd, ['^REST' num2str( length( Runs)) '.*.nii.gz']);
end

disp( [[ 'cat' num2str( length( Runs)) ], num2str( TR), out, num2str( MaxLag), num2str( MinR), num2str( Fixed), num2str( Sm)])
